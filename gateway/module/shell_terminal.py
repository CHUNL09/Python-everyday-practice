...
def shell_terminal(gateway_user_obj,host_user_obj):
    try:
        ...
        client.connect(host_user_obj.host.ip_addr,host_user_obj.host.port,
                        host_user_obj.user,host_user_obj.password,)
        log_msg='login [%s]' %(host_user_obj.host.hostname)
        logger.log_record(gateway_user_obj.id,host_user_obj.id,log_msg)
        chan = client.invoke_shell()
        print(repr(client.get_transport()))
        print('*** Here we go!\n')
        interactive.interactive_shell(chan,gateway_user_obj,host_user_obj)  # 补全参数
        chan.close()
        client.close()
