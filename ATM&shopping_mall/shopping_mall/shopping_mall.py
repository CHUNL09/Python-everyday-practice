...
from ATM.atm_pub_interface import atm_pub_interface as atm
        ...
        while True:
            ...
            if choice.strip()=='y' or choice.strip()=='Y':    # 表示要充值
                ...
                if input_money.isnumeric():   # 表示输入的是有效地数字（非负数）
                    if atm.pay(input_money,"商城充值")=="success":
        ...
        while True:   #用于购物模块和结算模块的循环，比如结算模块选择继续购物后会进入购物模块
            ...
            while True:
                ...
                ###################### 结算子模块 ##############
                    ...
                    user_input=input("使用信d用卡付款[credit]或者账户余额付款[account]：")
                    if user_input.strip()=="account":
                    ...
                    elif user_input.strip(' ')=="credit":
                        if atm.pay(cost,"商城消费")=="success":
                            print("结算成功！您已成功购买商品！")
                            finish_shopping=True
                    else:
                        pass
